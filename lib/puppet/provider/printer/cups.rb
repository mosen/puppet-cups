...
Puppet::Type.type(:printer).provide :cups, :parent => Puppet::Provider do
  ...
  The PPD file reported by cups might have been reformatted by cups, so it's difficult to identify whether our changes
  ...
  def self.instances
    ...
    @prefetched_printers = prefetched_long
  ...
  def flush
    debug 'Flush printer settings as lpadmin execs'
    @property_hash.clear
  end
end